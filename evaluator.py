...
EVAL_METRICS = {
    # Æ½¾ù¾ø¶ÔÎó²î£¨Mean Absolute Error£©1/n * ¡Æ|Ygt - Ypred|
...
def evaluate(_model, checkpoint_inner_path, tag="default", checkpoint_parent_name=None, eval_dataset=None,
    ...
    height_map = model.optical_system.doe_layer.preprocess_height_map(training=False, testing=True)
    height_map = tf.squeeze(height_map, axis = -1)
    height_map = tf.squeeze(height_map, axis = 0 )
    print("Saving height map to MAT...")
    sio.savemat(
        "./exported-mat-files/%s-height-map-%s.mat" % (datetime.now().strftime("%Y%m%d-%H%M%S"), output_name),
        {"height_map": height_map.numpy()}, do_compression=False)
    
    print("DOE height map has successfully exported.")
